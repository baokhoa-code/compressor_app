...
public class VideoCompressedListActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    static CompressedVideosAdapter compressedVideosAdapter;
    private RecyclerView recyclerView;
    private SwipeRefreshLayout swipeRefreshLayout;
    private List<File> compressedVideoFiles;
    private ArrayList<HandleVideo> compressedHandleVideos;
    ...
    private void showCompressedVideos() {
        compressedVideoFiles = fetchCompressedVideoFiles();
        compressedHandleVideos = fetchCompressedHandleVideos();
        compressedVideosAdapter = new CompressedVideosAdapter(compressedHandleVideos, this, 2, new CompressedVideosAdapter.CompressedVideoListener() {
            @Override
        ...
        recyclerView.setAdapter(compressedVideosAdapter);
        recyclerView.setLayoutManager(new LinearLayoutManager(this,
                RecyclerView.VERTICAL, false));
        compressedVideosAdapter.notifyDataSetChanged();
    }

    private List<File> fetchCompressedVideoFiles() {
    ...
    private ArrayList<HandleVideo> fetchCompressedHandleVideos() {
        ArrayList<HandleVideo> videoFilesTemp = new ArrayList<>();

        for (File file : compressedVideoFiles) {
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        for (HandleVideo media : compressedHandleVideos) {
            if (media.getName().toLowerCase().contains(inputs)) {
                mediaFiles.add(media);
            }
        }
        VideoCompressedListActivity.compressedVideosAdapter.updateVideoFiles(mediaFiles);
        return true;
    }
